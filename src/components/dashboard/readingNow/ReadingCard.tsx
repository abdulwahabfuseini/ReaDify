...
import { ReadingBooksActions } from "@/redux/ReadingBooks";

const ReadingCard = () => {
  const ReadingNow = useSelector(selectFavoriteBooks);
          ...
          <div className="w-full">
            ...
            <div className="grid grid-cols-2 sm:flex gap-2 flex-wrap justify-center sm:justify-start items-center  w-full my-6 mb-20">
              {ReadingNow.map((reading) => (
                <div key={reading.id} className="border-2">
                  <Image
                    src={`/images/${reading?.imageLinks}`}
                    width={150}
                    height={250}
                    alt="cover"
                    quality={100}
